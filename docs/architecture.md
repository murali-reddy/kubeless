...
A _Function_ is representation of the code to be executed. Along with the code a _Function_ contains metadat about its runtime dependencies, build instructions etc. A _Function_ has a independent life-cycle. The following methods are supported:
...
* Update - Modify the function specification and its meta-data
...
* Update - Modify the triggeer specification
...
- Each event source is modelled as a seperate Trigger CRD object
- Seperate Custom Resource Definitions controller to handle CRUD operations corresponding to CRD object
...
Use of Kubernetes CRD's and CRD controllers forms the core design tenet of Kubeless. Use of seperate CRD's for functions and triggers provides clear seperation of concerns. Use of seperate CRD controlls keeps the code decoupled and modular.
